...
class OcrToTableTool:
    ...
    def club_all_bounding_boxes_by_similar_y_coordinates_into_rows(self):
        # if not self.bounding_boxes or not self.bounding_boxes[0]:
        #     return
        # self.rows = []
        ...
        for bounding_box in self.bounding_boxes[1:]:
            ...
            # Ensure there are elements in current_row before accessing the last element
            if current_row and len(current_row[-1]) == 4:
                distance_between_bounding_boxes = abs(current_bounding_box_y - previous_bounding_box_y)
                if distance_between_bounding_boxes <= half_of_mean_height:
                    current_row.append(bounding_box)
                else:
        ...
        #     # Dynamically determine the threshold for grouping bounding boxes into rows
        #     dynamic_threshold = min(half_of_mean_height, self.mean_height * 0.2)

        #     if distance_between_bounding_boxes <= dynamic_threshold:
        #         current_row.append(bounding_box)
        #     else:
        #         self.rows.append(current_row)
        #         current_row = [bounding_box]
    ...
    def get_result_from_tersseract(self, image_path):
        ...
        # output = output.strip()
        # return output
        # Specify the whitelist based on the expected characters
        whitelist = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789().calmg*/"

        # Construct the Tesseract command with additional parameters
        command = f'tesseract {image_path} - -l eng --oem 3 --psm 6 --dpi 72 -c tessedit_char_whitelist="{whitelist}"'

        # Run Tesseract and get the output
